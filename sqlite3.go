...
void gsc_callbackTrampoline(sqlite3_context*, int, sqlite3_value**);
void gsc_stepTrampoline(sqlite3_context*, int, sqlite3_value**);
void gsc_doneTrampoline(sqlite3_context*);

int gsc_compareTrampoline(void*, int, char*, int, char*);
int gsc_commitHookTrampoline(void*);
void gsc_rollbackHookTrampoline(void*);
void gsc_updateHookTrampoline(void*, int, char*, char*, sqlite3_int64);

int gsc_authorizerTrampoline(void*, int, char*, char*, char*, char*);
...
// go build -ldflags="-X 'github.com/sjzar/go-sqlcipher.driverName=my-sqlite3'"
var driverName = "sqlcipher"
...
func (c *SQLiteConn) RegisterCollation(name string, cmp func(string, string) int) error {
	...
	rv := C.sqlite3_create_collation(c.db, cname, C.SQLITE_UTF8, handle, (*[0]byte)(unsafe.Pointer(C.gsc_compareTrampoline)))
	if rv != C.SQLITE_OK {
		return c.lastError()
	}
	return nil
...
func (c *SQLiteConn) RegisterCommitHook(callback func() int) {
	if callback == nil {
		...
		C.sqlite3_commit_hook(c.db, (*[0]byte)(C.gsc_commitHookTrampoline), newHandle(c, callback))
	}
}
...
func (c *SQLiteConn) RegisterRollbackHook(callback func()) {
	if callback == nil {
		...
		C.sqlite3_rollback_hook(c.db, (*[0]byte)(C.gsc_rollbackHookTrampoline), newHandle(c, callback))
	}
}
...
func (c *SQLiteConn) RegisterUpdateHook(callback func(int, string, string, int64)) {
	if callback == nil {
		...
		C.sqlite3_update_hook(c.db, (*[0]byte)(C.gsc_updateHookTrampoline), newHandle(c, callback))
	}
}
...
func (c *SQLiteConn) RegisterAuthorizer(callback func(int, string, string, string) int) {
	if callback == nil {
		...
		C.sqlite3_set_authorizer(c.db, (*[0]byte)(C.gsc_authorizerTrampoline), newHandle(c, callback))
	}
}
...
func (c *SQLiteConn) RegisterFunc(name string, impl any, pure bool) error {
	...
	rv := sqlite3CreateFunction(c.db, cname, C.int(numArgs), C.int(opts), newHandle(c, &fi), C.gsc_callbackTrampoline, nil, nil)
	if rv != C.SQLITE_OK {
		return c.lastError()
	}
	return nil
...
func (c *SQLiteConn) RegisterAggregator(name string, impl any, pure bool) error {
	...
	rv := sqlite3CreateFunction(c.db, cname, C.int(stepNArgs), C.int(opts), newHandle(c, &ai), nil, C.gsc_stepTrampoline, C.gsc_doneTrampoline)
	if rv != C.SQLITE_OK {
		return c.lastError()
	}
	return nil
...
func (c *SQLiteConn) exec(ctx context.Context, query string, args []driver.NamedValue) (driver.Result, error) {
	...
	for {
		...
		if tail == "" {
			if res == nil {
				// https://github.com/sjzar/go-sqlcipher/issues/963
